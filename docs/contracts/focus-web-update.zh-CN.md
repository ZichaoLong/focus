...
updater 在 journal 中持久化受限的固定 `phase`、阶段开始时间、最近实际进展时间和可选字节进度。
Stable 下载阶段在响应提供大小时报告已收/总字节；Git/npm/pip 的原始输出不进入浏览器或公共 journal，
以避免凭据 URL 泄漏。阶段计时是诊断事实，不代表可以安全地按百分比推断未报告的构建或依赖步骤。
