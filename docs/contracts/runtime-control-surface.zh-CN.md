...
- 当前目录
- 当前线程
- 当前 thread 的 goal 摘要（如果当前投影已知）
- 当前 thread 的 thread-wise profile
- 当前飞书会话后续 turn 的 model / effort / 权限 / 审批 / 沙箱 / 协作模式

它不再承担完整调试面。

### 5.2 `/goal`

`/goal` 是当前 binding 所指向 thread 的 goal 控制面。
...
- 无参数 `/goal` 等价于 `/goal show`
- 这里依赖的是 app-server 的 `thread/goal/*` RPC，而不是复刻 TUI slash 交互
- goal 状态是 thread-level 展示面，不进入 execution card 状态机
- 即使一个 goal 连续驱动多个 turn，飞书侧仍按普通逐 turn 执行卡生命周期展示
