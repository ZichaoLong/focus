...
| `/profile [name]` | 查看或切换当前实例 managed backend 的 startup profile | 是；`线程设置` 页 | 仅管理员 | 仅管理员 | 无直接本地等价命令 |
| `/profile-clear` | 清空当前实例 managed backend 的 startup profile override | 否；卡片按钮也可触发 | 仅管理员 | 仅管理员 | 无直接本地等价命令 |
