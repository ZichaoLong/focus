...
| `feishu-codexctl [--instance <name>] binding clear-stale [--dry-run]` | 清理指向已不可读出可恢复 history 的 stale binding bookmark；默认扫描所有运行中实例和已知非运行实例，显式 `--instance` 时只作用于该实例 | 变更 | 无；这是本地 bookmark 修复 / 运维入口 |
...
- 它先通过运行中的 app-server 验证 binding 指向的 `current_thread_id` 是否仍能读出可恢复的 thread history。
- 能读出 history 的 thread 视为保留对象；即使普通状态是 `notLoaded`，只要 history 可读，也不是 stale。
- 明确不可读、未加载且无持久 history、或只剩不可恢复 metadata 的 thread 视为 stale，清理对应本地 bookmark。
