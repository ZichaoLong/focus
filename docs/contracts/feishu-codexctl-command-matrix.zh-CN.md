...
| `feishu-codexctl [--instance <name>] thread archive (--thread-id <id> \| --thread-name <name>)` | 归档目标 thread，使其从常规列表中隐藏，并清理当前目标实例里仍指向它的 bindings | thread-scoped 归档 + 当前实例 binding 清理 | 变更 | 必须二选一：`--thread-id` 或 `--thread-name`；若省略 `--instance`，CLI 在已知 live runtime owner 时可优先路由到该实例 | 最接近飞书 `/archive` 的本地对应，但它是实例定向的，不是当前 chat-scoped |
