...
1. bare `pytest` 现在可以直接工作，直接原因是 [pyproject.toml](../../pyproject.toml:38) 里新增了：
...
- [tests/test_binding_runtime_manager.py](../../tests/test_binding_runtime_manager.py:238)
- [tests/test_binding_runtime_manager.py](../../tests/test_binding_runtime_manager.py:266)
- [tests/test_binding_runtime_manager.py](../../tests/test_binding_runtime_manager.py:495)
- [tests/test_binding_runtime_manager.py](../../tests/test_binding_runtime_manager.py:590)
- [tests/test_runtime_admin_controller.py](../../tests/test_runtime_admin_controller.py:1201)
...
- [bot/runtime_admin_controller.py](../../bot/runtime_admin_controller.py:550)
- [bot/runtime_admin_controller.py](../../bot/runtime_admin_controller.py:568)
- [bot/binding_runtime_manager.py](../../bot/binding_runtime_manager.py:494)
- [bot/codex_handler.py](../../bot/codex_handler.py:365)
...
- [install.ps1](../../install.ps1:5)
- [bot/platform_paths.py](../../bot/platform_paths.py:115)
...
- [bot/fcodex_proxy.py](../../bot/fcodex_proxy.py:1009)
- [tests/test_codex_app_server.py](../../tests/test_codex_app_server.py:2115)
...
1. wrapper 先启动本地 cwd proxy [bot/fcodex.py](../../bot/fcodex.py:743)
2. upstream 连接这个 proxy
3. 第一次连接意外断开
4. proxy 在没有活跃连接时启动 idle watchdog [bot/fcodex_proxy.py](../../bot/fcodex_proxy.py:1039)
5. 如果第二次连接还没来得及进入 handler、watchdog 就先执行了 `server.shutdown()`，就会在客户端看到 `ConnectionRefusedError` [bot/fcodex_proxy.py](../../bot/fcodex_proxy.py:1049)

也就是说，坏结果发生在 websocket 握手前，属于“监听已经关掉”，不是 resume payload 注入 profile / memory mode 的逻辑错误。

这里要把两个不同超时分开理解：

1. 当前这条观察真正相关的是 proxy idle window，默认值是 [bot/fcodex_proxy.py](../../bot/fcodex_proxy.py:58) 的 `30.0s`
2. [bot/fcodex.py](../../bot/fcodex.py:720) 里还有一个 `5.0s` 的 proxy readiness timeout，但那是 wrapper 等待子进程打印 listen url 的启动超时，不是 `resume` 两次 websocket 连接之间的 grace window
...
1. 单独跑 [tests/test_codex_app_server.py](../../tests/test_codex_app_server.py:1)，峰值 RSS 约 `1.70 GB`
2. 单独跑 [tests/test_codex_handler.py](../../tests/test_codex_handler.py:1)，峰值 RSS 约 `210 MB`
3. 单独跑 [tests/test_manage_cli.py](../../tests/test_manage_cli.py:1) + [tests/test_runtime_admin_controller.py](../../tests/test_runtime_admin_controller.py:1)，峰值 RSS 约 `181 MB`

继续把 [tests/test_codex_app_server.py](../../tests/test_codex_app_server.py:1) 按测试类拆分后，最可疑的是 [tests/test_codex_app_server.py](../../tests/test_codex_app_server.py:779) `CodexRpcClientTests`：

- 单独跑这 7 条测试，峰值 RSS 约 `604 MB`
- 其中只跑 [tests/test_codex_app_server.py](../../tests/test_codex_app_server.py:828) 这一条，峰值 RSS 约 `262 MB`
...
1. `40331f5` 已把 `detach_binding_locked(...)`、`detach_thread_bindings_locked(...)`、`deactivate_binding_locked(...)`、`deactivate_bindings_locked(...)` 改成 staged-persist-then-commit 顺序，关键实现集中在 [bot/binding_runtime_manager.py](../../bot/binding_runtime_manager.py:417) 和 [bot/binding_runtime_manager.py](../../bot/binding_runtime_manager.py:551)。
...
- [bot/binding_runtime_manager.py](../../bot/binding_runtime_manager.py:494) 的 `binding_keys_locked()` 只代表当前 runtime map，不代表持久化 store 的全量事实。
...
因为 [bot/codex_handler.py](../../bot/codex_handler.py:365) 启动时会 hydrate 全量 store，所以这条风险主要打在**恢复、修复、清场**场景，而不是普通新装后的第一条命令。
...
- [tests/test_manage_cli.py](../../tests/test_manage_cli.py:1009)
...
- [bot/fcodex_proxy.py](../../bot/fcodex_proxy.py:1049)
- [tests/test_codex_app_server.py](../../tests/test_codex_app_server.py:2115)
...
- [tests/test_codex_app_server.py](../../tests/test_codex_app_server.py:828)
- [bot/codex_protocol/client.py](../../bot/codex_protocol/client.py:224)
- [bot/codex_protocol/client.py](../../bot/codex_protocol/client.py:418)
...
- [tests/test_instance_registry_store.py](../../tests/test_instance_registry_store.py:95)
- [tests/test_thread_runtime_lease_store.py](../../tests/test_thread_runtime_lease_store.py:253)

如果按这个方向修测试，原则上不需要牺牲原有测试能力：

1. 对 [tests/test_codex_app_server.py](../../tests/test_codex_app_server.py:828) 而言，它真正想看护的是“默认 `codex` 命令不可用时，是否退回稳定启动命令，并拼出正确 argv”，不是日志线程是否无限读取 `MagicMock`
...
1. 对 [tests/test_codex_app_server.py](../../tests/test_codex_app_server.py:828) 这类只关心 `Popen` argv 的测试，改用显式 `_Proc` stub，并让 `stdout = StringIO(\"\")`、`stderr = StringIO(\"\")`
...
当前工作树里，[bot/runtime_admin_controller.py](../../bot/runtime_admin_controller.py:550) 在执行 `clear-all` 前，会先通过 [bot/binding_runtime_manager.py](../../bot/binding_runtime_manager.py:382) 把 store-only bindings 补 hydrate 到运行态，再沿用现有 batch deactivate 事务路径统一清除。

新增回归覆盖：

- [tests/test_runtime_admin_controller.py](../../tests/test_runtime_admin_controller.py:1239) 补了“1 个 live binding + 1 个 store-only stale binding”的恢复场景
...
1. [tests/test_codex_app_server.py](../../tests/test_codex_app_server.py:828) 不再让默认 `MagicMock` 驱动真实日志线程，而是改成显式 EOF 的 `Popen` stub + no-op 线程替身
2. [tests/test_instance_registry_store.py](../../tests/test_instance_registry_store.py:100) 与 [tests/test_thread_runtime_lease_store.py](../../tests/test_thread_runtime_lease_store.py:258) 的并发进程测试不再从多线程父进程里继续 `fork`，而是改成 `spawn`

我在当前工作树重新测得：

1. 单独跑 [tests/test_codex_app_server.py](../../tests/test_codex_app_server.py:828) 该可疑测试，峰值 RSS 约 `46 MB`
2. 单独跑 [tests/test_codex_app_server.py](../../tests/test_codex_app_server.py:1)，峰值 RSS 约 `46 MB`
