...
# 新线程默认 thread-wise memory mode；默认值：空（不额外注入）
...
# default_thread_memory_mode: read

# Codex 可执行命令；默认值：codex
# 首次安装时，如检测到 fnm / nvm 管理的 Codex，会自动把稳定启动命令写入真实 `codex.yaml`。
...
def render_initial_codex_yaml() -> str:
    ...
    return CODEX_YAML_TEMPLATE.replace(
        ...
        "\n".join(
            [
                "# 已自动探测到稳定的 Node 管理器 Codex 启动命令；如需改回其他命令，可手动编辑。",
                rendered_assignment,
                "# codex_command: codex",
            ]
        ),
